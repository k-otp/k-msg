---
editUrl: false
next: false
prev: false
title: "FaxMessageSendOptions"
---

Defined in: [packages/core/src/types/message.ts:304](https://github.com/k-otp/k-msg/blob/main/packages/core/src/types/message.ts#L304)

## Extends

- [`CommonSendOptions`](/en/api/core/src/interfaces/commonsendoptions/)

## Properties

### fax

> **fax**: [`FaxSendOptions`](/en/api/core/src/interfaces/faxsendoptions/)

Defined in: [packages/core/src/types/message.ts:306](https://github.com/k-otp/k-msg/blob/main/packages/core/src/types/message.ts#L306)

***

### from?

> `optional` **from**: `string`

Defined in: [packages/core/src/types/message.ts:146](https://github.com/k-otp/k-msg/blob/main/packages/core/src/types/message.ts#L146)

Sender number / sender id. Optional at KMsg layer; providers may require it.

#### Inherited from

[`CommonSendOptions`](/en/api/core/src/interfaces/commonsendoptions/).[`from`](/en/api/core/src/interfaces/commonsendoptions/#from)

***

### messageId?

> `optional` **messageId**: `string`

Defined in: [packages/core/src/types/message.ts:133](https://github.com/k-otp/k-msg/blob/main/packages/core/src/types/message.ts#L133)

Correlation id generated by KMsg (or provided by the caller).
Providers must echo this value back in SendResult.messageId.

#### Inherited from

[`CommonSendOptions`](/en/api/core/src/interfaces/commonsendoptions/).[`messageId`](/en/api/core/src/interfaces/commonsendoptions/#messageid)

***

### options?

> `optional` **options**: `object`

Defined in: [packages/core/src/types/message.ts:150](https://github.com/k-otp/k-msg/blob/main/packages/core/src/types/message.ts#L150)

Common delivery options understood by multiple providers.

#### country?

> `optional` **country**: `string`

Country code for providers that support it (e.g. SOLAPI).
Examples: "82", "+82".

#### customFields?

> `optional` **customFields**: `Record`\<`string`, `string`\>

#### scheduledAt?

> `optional` **scheduledAt**: `Date`

#### Inherited from

[`CommonSendOptions`](/en/api/core/src/interfaces/commonsendoptions/).[`options`](/en/api/core/src/interfaces/commonsendoptions/#options)

***

### providerId?

> `optional` **providerId**: `string`

Defined in: [packages/core/src/types/message.ts:137](https://github.com/k-otp/k-msg/blob/main/packages/core/src/types/message.ts#L137)

Optional routing hint to force a specific provider by id.

#### Inherited from

[`CommonSendOptions`](/en/api/core/src/interfaces/commonsendoptions/).[`providerId`](/en/api/core/src/interfaces/commonsendoptions/#providerid)

***

### providerOptions?

> `optional` **providerOptions**: `Record`\<`string`, `unknown`\>

Defined in: [packages/core/src/types/message.ts:162](https://github.com/k-otp/k-msg/blob/main/packages/core/src/types/message.ts#L162)

Provider-specific escape hatch (use sparingly).

#### Inherited from

[`CommonSendOptions`](/en/api/core/src/interfaces/commonsendoptions/).[`providerOptions`](/en/api/core/src/interfaces/commonsendoptions/#provideroptions)

***

### to

> **to**: `string`

Defined in: [packages/core/src/types/message.ts:142](https://github.com/k-otp/k-msg/blob/main/packages/core/src/types/message.ts#L142)

Recipient phone number in Korean format without hyphens.

#### Example

```ts
"01012345678"
```

#### Inherited from

[`CommonSendOptions`](/en/api/core/src/interfaces/commonsendoptions/).[`to`](/en/api/core/src/interfaces/commonsendoptions/#to)

***

### type

> **type**: `"FAX"`

Defined in: [packages/core/src/types/message.ts:305](https://github.com/k-otp/k-msg/blob/main/packages/core/src/types/message.ts#L305)
