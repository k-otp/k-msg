...
> `const` **ChannelFiltersSchema**: `ZodObject`\<\{ `createdAfter`: `ZodOptional`\<`ZodDate`\>; `createdBefore`: `ZodOptional`\<`ZodDate`\>; `provider`: `ZodOptional`\<`ZodString`\>; `status`: `ZodOptional`\<`ZodEnum`\<*typeof* [`ChannelStatus`](/api/channel/src/toolkit/enumerations/channelstatus/)\>\>; `type`: `ZodOptional`\<`ZodEnum`\<*typeof* [`ChannelType`](/api/channel/src/toolkit/enumerations/channeltype/)\>\>; `verified`: `ZodOptional`\<`ZodBoolean`\>; \}, `$strip`\>
