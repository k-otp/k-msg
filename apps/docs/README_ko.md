...
- 소스 콘텐츠 워크스페이스: `apps/docs`는 plain Markdown 원본, 생성 입력, 예제 스니펫 원본을 관리합니다
...
- 패키지 `exports`를 읽어 문서 파이프라인이 사용하는 API source inventory를 생성
...
- 추적 중인 schema reference 페이지를 문서 소스 트리로 동기화
...
- `cli`, `snippets`용 plain-Markdown 정적 페이지 재생성
...
- 문서가 plain-Markdown 호환 상태를 유지하는지 검증하고 Hono 앱 콘텐츠 트리로 복사

8. `scripts/docs/generate-api-extractor-docs.ts`
- `apps/docs/api-sources.json` inventory를 기반으로 API Extractor + API Documenter 문서 생성
...
- `bun run scripts/docs/run-generate.ts --check`는 `scripts/docs/export-hono-content.ts`와 `scripts/docs/generate-api-extractor-docs.ts`도 의도적으로 제외합니다.
...
- `bun run docs-hono:build` 완료 시 `sitemap-index.xml`이 `apps/docs-hono/dist`에 생성됩니다.
- sitemap 메타데이터는 `scripts/docs/postprocess-sitemap.ts`가 후처리하고 `scripts/docs/check-sitemap-lastmod.ts`가 검증합니다.
- `apps/docs-hono/public/sitemap.xsl`과 Hono 빌드가 canonical sitemap 자산을 함께 생성합니다.
