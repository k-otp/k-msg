...
class KMessageService {
  ...
  async loadProviderTemplates() {
    try {
      ...
      if (Array.isArray(templates)) {
        ...
        console.log(
          `📋 Loaded ${this.providerTemplates.length} provider templates`,
        );
      } else {
  ...
  async listTemplates(source: "local" | "provider" | "all" = "all") {
    try {
      ...
      if (source === "all") {
        ...
        result.templates =
          source === "local" ? result.localTemplates : result.providerTemplates;
      }
  ...
  async getAnalytics() {
    try {
      ...
      const successfulMessages = this.messages.filter(
        (m) => m.result.messageId,
      ).length;
      const successRate =
...
console.log(
  `📦 Using packages: core, provider, template, messaging, analytics`,
);
