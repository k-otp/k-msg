...
export class BatchDispatcher extends EventEmitter {
  ...
  getBatchStats(): {
    ...
    return {
      ...
      averageQueueSize:
        endpointIds.length > 0 ? totalPendingJobs / endpointIds.length : 0,
    };
  }
  ...
  private async executeBatch(
    ...
    try {
      ...
      if (failed > 0) {
        // 일부 실패가 있으면 에러로 처리
        throw new Error(
          `Batch partially failed: ${failed}/${deliveries.length} jobs failed`,
        );
      }
    } catch (error) {
  ...
  private requeueFailedJobs(jobs: DispatchJob[]): void {
    for (const job of jobs) {
      ...
      if (job.attempts < job.maxAttempts) {
        ...
        const delay = baseDelay * backoffMultiplier ** (job.attempts - 1);
