...
const createTestEndpoint = (
  events: WebhookEventType[] = [WebhookEventType.MESSAGE_SENT],
...
describe("WebhookService", () => {
  ...
  test("should update webhook endpoint", async () => {
    ...
    const updatedEndpoint = await webhookService.updateEndpoint(
      endpoint.id,
      updates,
    );
    ...
    expect(updatedEndpoint.updatedAt.getTime()).toBeGreaterThan(
  ...
  test("should get webhook statistics", async () => {
    ...
    const stats = await webhookService.getStats(endpoint.id, {
      start: oneHourAgo,
      end: now,
    });
...
describe("SecurityManager", () => {
  ...
  test("should generate and verify signatures", () => {
    ...
    const isInvalid = securityManager.verifySignature(
      payload,
      "invalid-signature",
      secret,
    );
...
describe("EndpointManager", () => {
  ...
  test("should search endpoints with filters", async () => {
    ...
    expect(sentEventEndpoints.items[0].events).toContain(
      WebhookEventType.MESSAGE_SENT,
    );
  });
  ...
  test("should get active endpoints for specific event", async () => {
    ...
    const activeEndpoints = await endpointManager.getActiveEndpointsForEvent(
      WebhookEventType.MESSAGE_SENT,
    );
  ...
  test("should get endpoint statistics", async () => {
    ...
    expect(stats.eventSubscriptions[WebhookEventType.MESSAGE_DELIVERED]).toBe(
      1,
    );
  });
});
...
describe("Integration Tests", () => {
  ...
  test("should handle error scenarios gracefully", async () => {
    ...
    try {
      ...
      await expect(
        webhookService.updateEndpoint("non-existent-id", { name: "Test" }),
      ).rejects.toThrow("not found");

      // 존재하지 않는 엔드포인트 테스트 시도
      await expect(
        webhookService.testEndpoint("non-existent-id"),
      ).rejects.toThrow("not found");
