  ...
  implements BaseProvider<StandardRequest, StandardResult>
{
  ...
  configure(config: Record<string, unknown>): void {
    ...
    const AdapterClass = this.adapter.constructor as new (
  ...
  async healthCheck(): Promise<ProviderHealthStatus> {
    ...
    try {
      ...
      try {
        ...
        issues.push(
          `Authentication configuration error: ${error instanceof Error ? error.message : "Unknown"}`,
        );
      }
  ...
  >(request: T): Promise<R> {
    ...
    try {
      ...
      const errorResult: StandardResult = {
        messageId:
          this.adapter["generateMessageId"]?.() || `error_${Date.now()}`,
