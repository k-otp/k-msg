...
export const KMessageConfigSchema = z.object({
  ...
  server: ServerConfigSchema.default({ port: 3000, host: 'localhost', cors: { origins: ['*'], credentials: false }, maxRequestSize: 10485760, requestTimeout: 30000, enableMetrics: true, enableDocs: true }),

  database: DatabaseConfigSchema.optional(),
  redis: RedisConfigSchema.optional(),

  logger: LoggerConfigSchema.default({ level: LogLevel.INFO, enableConsole: true, enableFile: false, maxFileSize: 10485760, maxFiles: 5, enableJson: false, enableColors: true }),
  health: HealthConfigSchema.default({ timeout: 5000, interval: 30000, retries: 3, includeMetrics: true }),
  queue: QueueConfigSchema.default({ maxConcurrency: 10, maxRetries: 3, retryDelay: 1000, maxAge: 3600000, batchSize: 100, processingInterval: 5000 }),
  ...
  }).default({ enableBulkSending: true, enableScheduling: false, enableAnalytics: true, enableWebhooks: false, enableTemplateCache: true, maxTemplatesPerProvider: 1000, maxRecipientsPerBatch: 1000 }),
  ...
  }).default({ apiKeyRequired: true, rateLimitEnabled: true, maxRequestsPerMinute: 1000, enableCors: true, trustedProxies: [] })
});
    ...
    const config = {
      ...
      providers: {
        ...
        sms: process.env.SMS_PROVIDER ? {
          provider: process.env.SMS_PROVIDER as 'iwinv' | 'aligo' | 'coolsms',
...
// Utility functions
export function mergeConfigs(base: KMessageConfig, override: Partial<KMessageConfig>): KMessageConfig {
  return KMessageConfigSchema.parse({
    ...base,
    ...override,
...
export function validateProviderConfig(provider: 'iwinv' | 'sms', config: any): any {
  switch (provider) {
    case 'iwinv':
