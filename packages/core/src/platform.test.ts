...
import type { BaseProvider, Config, StandardRequest, StandardResult } from './types/index';

class MockBaseProvider implements BaseProvider<StandardRequest, StandardResult> {
  ...
  async send<T extends StandardRequest = StandardRequest, R extends StandardResult = StandardResult>(request: T): Promise<R> {
...
class FailedResultProvider extends MockBaseProvider {
  async send<T extends StandardRequest = StandardRequest, R extends StandardResult = StandardResult>(_request: T): Promise<R> {
    return {
      isSuccess: false,
      isFailure: true,
...
describe('AlimTalkPlatform unified send API', () => {
  test('keeps legacy templateId API compatibility', async () => {
    ...
    const provider = new MockBaseProvider('default-provider', 'Default Provider');
    platform.registerProvider(provider);

    const result = await platform.messages.send({
      templateId: 'WELCOME_001',
      recipients: [{ phoneNumber: '01012345678', variables: { name: '홍길동' } }],
  ...
  test('selects provider from channel defaults', async () => {
    ...
    const defaultProvider = new MockBaseProvider('default-provider', 'Default Provider');
  ...
  test('throws for missing templateCode on template-based channels', async () => {
    ...
    await expect(platform.messages.send({
      channel: 'ALIMTALK',
  ...
  test('counts FAILED provider results as failed without exceptions', async () => {
    const platform = new AlimTalkPlatform(createConfig());
    const provider = new FailedStatusProvider('default-provider', 'Default Provider');
    platform.registerProvider(provider);
  ...
  test('counts legacy Result failure payload as failed', async () => {
    const platform = new AlimTalkPlatform(createConfig());
    const provider = new FailedResultProvider('default-provider', 'Default Provider');
    platform.registerProvider(provider);
    ...
    expect(result.summary.failed).toBe(1);
    expect(result.results[0]?.status).toBe('FAILED');
    expect(result.results[0]?.error?.message).toContain('legacy result failure');
  });
});
