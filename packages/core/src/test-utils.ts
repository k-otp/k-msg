...
import { Provider, SendOptions, SendResult, KMsgError, KMsgErrorCode, Result, ok, fail } from './index';
...
export class MockProvider implements Provider {
  ...
  async send(params: SendOptions): Promise<Result<SendResult, KMsgError>> {
    if (params.to === '01000000000') {
      return fail(new KMsgError(KMsgErrorCode.INVALID_REQUEST, 'Invalid phone number'));
    }
