...
export class AnomalyDetector {
  ...
  async detectBatchAnomalies(
    ...
    for (const [key, typeMetrics] of groupedMetrics.entries()) {
      ...
      // 각 포인트에 대해 이상 탐지
      for (
        let i = Math.floor(adjustedSeries.length * 0.3);
        i < adjustedSeries.length;
        i++
      ) {
  ...
  async detectTrendChanges(
    ...
    for (const [key, typeMetrics] of groupedMetrics.entries()) {
      ...
      for (let i = windowSize; i < sortedMetrics.length - windowSize; i++) {
        ...
        const beforeTrend = this.calculateTrend(
          beforeWindow.map((m) => m.aggregations.avg),
        );
        const afterTrend = this.calculateTrend(
          afterWindow.map((m) => m.aggregations.avg),
        );
        ...
        if (trendChange > significanceThreshold) {
          insights.push({
            ...
            confidence: Math.min(
              0.95,
              0.5 + (trendChange / significanceThreshold) * 0.3,
            ),
  ...
  private async runAlgorithm(
    ...
    switch (algorithm.name) {
      ...
      case "isolation":
        return this.isolationForestDetection(
          metric,
          historicalValues,
          algorithm.config,
          key,
      ...
      return {
        ...
        context: {
          ...
          historicalComparison:
            ((metric.value - expectedValue) / expectedValue) * 100,
        },
      };
    }
  ...
  private isolationForestDetection(
    ...
    const threshold = this.calculateIsolationThreshold(
      historicalValues,
      contamination,
    );

    const anomalyScore = this.calculateIsolationScore(
      metric.value,
      historicalValues,
    );
  ...
  private deduplicateAndRankAnomalies(anomalies: Anomaly[]): Anomaly[] {
    ...
    for (const anomaly of anomalies) {
      ...
      if (
        !unique.has(key) ||
        unique.get(key)!.confidence < anomaly.confidence
      ) {
  ...
  private generateTrendRecommendations(
    ...
    if (change > 2.0) {
      recommendations.push(
        "Consider immediate investigation due to significant change",
      );
    }
