...
export class DashboardGenerator {
  ...
  async generateDashboard(
    ...
    try {
      ...
      // 위젯 데이터 생성
      dashboard.widgets = await this.generateWidgetData(
        metrics,
        timeRange,
        filters,
      );
  ...
  async *streamDashboard(
    ...
    while (true) {
      ...
      await new Promise((resolve) =>
        setTimeout(resolve, this.config.refreshInterval),
      );
    }
  }
  ...
  private async calculateKPIs(
    ...
    // 메시지 전송량 KPI
    const sentMetrics = this.filterMetrics(
      metrics,
      MetricType.MESSAGE_SENT,
      filters,
    );
    const totalSent = sentMetrics.reduce(
      (sum, m) => sum + m.aggregations.sum,
      0,
    );
    const previousSent = this.calculatePreviousPeriodValue(
      sentMetrics,
      previousTimeRange,
    );
    ...
    kpis.push({
      ...
      changePercent:
        previousSent > 0
          ? ((totalSent - previousSent) / previousSent) * 100
          : 0,
      trend:
    ...
    // 전달률 KPI
    const deliveredMetrics = this.filterMetrics(
      metrics,
    ...
    // 오류율 KPI
    const failedMetrics = this.filterMetrics(
      metrics,
      MetricType.MESSAGE_FAILED,
      filters,
    );
    const totalFailed = failedMetrics.reduce(
      (sum, m) => sum + m.aggregations.sum,
      0,
    );
    ...
    kpis.push({
      ...
      target: 5,
      status:
        errorRate <= 5 ? "good" : errorRate <= 15 ? "warning" : "critical",
    });

    // 클릭률 KPI (전달된 메시지 기준)
    const clickedMetrics = this.filterMetrics(
      metrics,
      MetricType.MESSAGE_CLICKED,
      filters,
    );
    const totalClicked = clickedMetrics.reduce(
      (sum, m) => sum + m.aggregations.sum,
      0,
    );
    const clickRate =
  ...
  private async generateWidgetData(...args: any[]): Promise<WidgetData[]> {
    ...
    for (const widget of widgets) {
      try {
        const data = await this.generateSingleWidgetData(
          widget,
          metrics,
          timeRange,
          filters,
  ...
  private async generateSingleWidgetData(
    ...
    const cacheExpiration =
      widget.refreshInterval || this.config.refreshInterval;
    ...
    // 메트릭 필터링
    const filteredMetrics = this.applyQueryFilters(
      metrics,
      widget.query,
      filters,
    );
    ...
    switch (widget.type) {
      case "metric":
        data = this.generateMetricWidgetData(
          filteredMetrics,
          widget.visualization,
        );
        break;
      case "chart":
        data = this.generateChartWidgetData(
          filteredMetrics,
          widget.visualization,
        );
        break;
      case "table":
        data = this.generateTableWidgetData(
          filteredMetrics,
          widget.visualization,
        );
        break;
      case "gauge":
        data = this.generateGaugeWidgetData(
          filteredMetrics,
          widget.visualization,
        );
        break;
      case "heatmap":
        data = this.generateHeatmapWidgetData(
          filteredMetrics,
          widget.visualization,
        );
        break;
      case "trend":
        data = this.generateTrendWidgetData(
          filteredMetrics,
          widget.visualization,
        );
        break;
  ...
  private generateMetricWidgetData(
    ...
    switch (aggregation) {
      ...
      case "avg":
        value =
          metrics.reduce((sum, m) => sum + m.aggregations.avg, 0) /
  ...
  private generateGroupedChartData(
    ...
    const series = Array.from(grouped.entries()).map(
      ([name, groupMetrics], index) => ({
        name,
        data: groupMetrics
        ...
        color:
          config.colors?.[index % (config.colors?.length || 1)] ||
  ...
  private generateTableWidgetData(
    ...
    for (const metric of metrics) {
      const key = groupBy
        .map((field) => metric.dimensions[field] || "Unknown")
        .join("|");
      if (!grouped.has(key)) {
        grouped.set(key, []);
      }
    ...
    const rows = Array.from(grouped.entries()).map(([key, groupMetrics]) => {
      ...
      row.avg =
        groupMetrics.reduce((sum, m) => sum + m.aggregations.avg, 0) /
        groupMetrics.length;
  ...
  private generateTrendWidgetData(
    ...
    const values = sortedMetrics.map((m) =>
      this.getAggregatedValue(m, config.aggregation || "avg"),
    );
  ...
  private calculatePreviousPeriodValue(
    ...
    const currentValue = metrics.reduce(
      (sum, m) => sum + m.aggregations.sum,
      0,
    );
