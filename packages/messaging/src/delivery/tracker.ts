...
export class DeliveryTracker extends EventEmitter {
  ...
  async trackMessage(
    ...
    const expiresAt = new Date(
      now.getTime() + this.options.maxTrackingDuration,
    );
  ...
  async updateStatus(
    ...
    record.deliveryReport.metadata = {
      ...record.deliveryReport.metadata,
      ...details.metadata,
    };
    ...
    if (details.deliveredAt)
      record.deliveryReport.deliveredAt = details.deliveredAt;
  ...
  getStatsForPeriod(startDate: Date, endDate: Date): DeliveryStats {
    ...
    records.forEach((record) => {
      ...
      if (record.deliveryReport.deliveredAt && record.deliveryReport.sentAt) {
        const deliveryTime =
          record.deliveryReport.deliveredAt.getTime() -
    ...
    if (records.length > 0) {
      stats.deliveryRate =
        (stats.byStatus[MessageStatus.DELIVERED] / records.length) * 100;
  ...
  cleanup(): number {
    ...
    for (const [messageId, record] of this.trackingRecords.entries()) {
      if (
        ...
        this.isTerminalStatus(record.currentStatus)
      ) {
  ...
  private async processTracking(): Promise<void> {
    ...
    const shouldCleanup =
      Date.now() % (60 * 60 * 1000) < this.options.trackingInterval;
    if (shouldCleanup) {
      this.cleanup();
    }
  }
  ...
  private updateStats(): void {
    ...
    if (this.stats.totalMessages > 0) {
      this.stats.deliveryRate =
        (this.stats.byStatus[MessageStatus.DELIVERED] /
