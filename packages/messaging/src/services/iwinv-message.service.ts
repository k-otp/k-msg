...
export class IWINVMessageService extends BaseMessageService {
  ...
  protected async defaultChannelLoader(): Promise<any[]> {
    try {
      ...
      const channels = await (this.iwinvProvider as any).channels.list();
  ...
  protected async defaultTemplateLoader(): Promise<any[]> {
    try {
      const templates = await (this.iwinvProvider as any).templates.list();
  ...
  async createTemplate(name: string, content: string, category: string) {
    try {
      ...
      try {
        await (this.iwinvProvider as any).createTemplate(name, content, category, variables);
  ...
  async sendMessage(phoneNumber: string, templateName: string, variables: Record<string, any>) {
    try {
      // 1. IWINV 메시지 발송 요청
      const result = await (this.iwinvProvider as any).sendMessage({
  ...
  async getIWINVBalance() {
    try {
      const balance = await (this.iwinvProvider as any).account.getBalance();
      return balance;
    } catch (error) {
      return {
        success: false,
