...
describe('KakaoChannelManager', () => {
  ...
  test('should validate Plus Friend ID format', async () => {
    ...
    await expect(manager.createChannel(invalidRequest)).rejects.toThrow('Invalid Plus Friend ID format');
  });
...
describe('KakaoSenderNumberManager', () => {
  test('should add and verify sender number', async () => {
    ...
    const verified = await manager.verifySenderNumber(senderNumber.id, senderNumber.verificationCode!);
  ...
  test('should reject invalid phone number format', async () => {
    ...
    await expect(manager.addSenderNumber('channel-1', {
  ...
  test('should handle verification code expiry', async () => {
    ...
    const verificationData = (manager as any).verificationCodes.get(senderNumber.id);
    if (verificationData) {
      verificationData.expiresAt = new Date(Date.now() - 1000); // 1 second ago
    }

    await expect(manager.verifySenderNumber(senderNumber.id, senderNumber.verificationCode!))
  ...
  test('should validate sender number for sending', async () => {
    ...
    // Before verification
    let validation = await manager.validateSenderNumberForSending(senderNumber.id);
    expect(validation.isValid).toBe(false);
    expect(validation.errors).toContain('Sender number status is VERIFYING, must be verified');

    // After verification
    await manager.verifySenderNumber(senderNumber.id, senderNumber.verificationCode!);
...
describe('ChannelCRUD', () => {
  test('should perform CRUD operations on channels', async () => {
    ...
    // Update
    const updated = await crud.updateChannel(channel.id, {
      name: '수정된 채널',
    ...
    expect(updated.updatedAt.getTime()).toBeGreaterThanOrEqual(channel.updatedAt.getTime());
  ...
  test('should list channels with filters and pagination', async () => {
    ...
    // Test filtering
    const alimtalkChannels = await crud.listChannels({ type: ChannelType.KAKAO_ALIMTALK });
    expect(alimtalkChannels.data.every(c => c.type === ChannelType.KAKAO_ALIMTALK)).toBe(true);
  });
...
describe('PermissionManager', () => {
  ...
  test('should prevent deletion of system roles', async () => {
    ...
    await expect(permissionManager.deleteRole('super-admin'))
...
describe('BusinessVerifier', () => {
  test('should submit business verification request', async () => {
    ...
    const request = await verifier.submitVerification('channel-1', businessInfo, documents);
  ...
  test('should validate required documents', async () => {
    const verifier = new BusinessVerifier({
      requiredDocuments: [DocumentType.BUSINESS_REGISTRATION, DocumentType.ID_CARD]
    });
    ...
    await expect(verifier.submitVerification('channel-1', businessInfo, documents))
  ...
  test('should manually approve verification', async () => {
    ...
    const approved = await verifier.approveVerification(request.id, 'reviewer-1', '모든 서류 확인 완료');
...
describe('NumberVerifier', () => {
  ...
  test('should verify correct code', async () => {
    ...
    const result = await verifier.verifyCode(request.id, request.verificationCode);
  ...
  test('should reject invalid phone numbers', async () => {
    ...
    await expect(verifier.startVerification('sender-1', '123-456-7890'))
  ...
  test('should handle rate limiting', async () => {
    ...
    await expect(verifier.startVerification('sender-2', '01012345678'))
  ...
  test('should get verification statistics', async () => {
    ...
    const request1 = await verifier.startVerification('sender-1', '01012345678');
    const request2 = await verifier.startVerification('sender-2', '01087654321');
