...
export class IWINVMultiProvider {
    ...
    switch (channel) {
      ...
      default:
        ...
        if (
          ...
          request.templateCode !== "SMS_DIRECT" &&
  ...
  async healthCheck() {
    ...
    if (alimtalkHealth.status === "rejected") {
      issues.push(
        `AlimTalk: ${alimtalkHealth.reason?.message || "Health check failed"}`,
      );
    ...
    return {
      ...
      data: {
        alimtalk:
          alimtalkHealth.status === "fulfilled" ? alimtalkHealth.value : null,
 ...
 */
export const createIWINVMultiProvider = (config: IWINVConfig) =>
  new IWINVMultiProvider(config);
