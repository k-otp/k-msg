...
export class IWINVProvider extends UniversalProvider {
  private static readonly directSmsTemplates = new Set(['SMS_DIRECT', 'LMS_DIRECT', 'MMS_DIRECT']);
  ...
  private isSmsChannelRequest(request: StandardRequest): boolean {
    ...
    const optionChannel =
      typeof request.options?.channel === 'string'
        ...
        : '';
    if (optionChannel === 'SMS' || optionChannel === 'LMS' || optionChannel === 'MMS') {
      return true;
    }
  ...
  private resolveSmsBaseUrl(): string {
    const explicitSmsBaseUrl =
      typeof this.iwinvConfig.smsBaseUrl === 'string' && this.iwinvConfig.smsBaseUrl.length > 0
        ? this.iwinvConfig.smsBaseUrl
        : this.iwinvConfig.baseUrl;

    let baseUrl = explicitSmsBaseUrl || 'https://sms.bizservice.iwinv.kr';
    baseUrl = baseUrl.replace('alimtalk.bizservice.iwinv.kr', 'sms.bizservice.iwinv.kr');
  ...
  private isLongMessage(request: StandardRequest, message: string): boolean {
    ...
    if (request.templateCode === 'LMS_DIRECT' || request.templateCode === 'MMS_DIRECT') {
      return true;
    }
    return message.length > 90;
  }

  private resolveSmsMessageType(request: StandardRequest, message: string): SmsV2MessageType {
    const optionType =
  ...
  private extractMessageText(request: StandardRequest): string {
    ...
    if (typeof request.variables.message === 'string' && request.variables.message.trim().length > 0) {
      return request.variables.message;
    }
    if (typeof request.variables._full_text === 'string' && request.variables._full_text.trim().length > 0) {
  ...
  private buildSmsSecretHeader(): string {
    ...
    const legacyAuthKey = this.iwinvConfig.smsAuthKey || this.iwinvConfig.smsApiKey;
    if (!legacyAuthKey) {
      return '';
    }
  ...
  private mapSmsErrorCode(code: string, responseOk: boolean): StandardErrorCode {
  ...
  private extractProviderCode(result: StandardResult): string {
    ...
    const nestedCode = this.normalizeCode((result.error?.details?.originalError as Record<string, unknown> | undefined)?.code);
    if (nestedCode) {
      return nestedCode;
    }
  ...
  private isIpRestrictionResult(result: StandardResult): boolean {
    ...
    return message.includes('등록하지 않은 IP') || message.toLowerCase().includes('unregistered ip');
  }
  ...
  private getIpRetryDelayMs(attempt: number): number {
    const raw = Number(this.iwinvConfig.ipRetryDelayMs ?? DEFAULT_IP_RETRY_DELAY_MS);
    if (!Number.isFinite(raw) || raw <= 0) {
  ...
  private async emitIpRestrictionAlert(
    ...
    const message = result.error?.message || '등록하지 않은 IP에서는 발송되지 않습니다.';
    ...
    if (typeof this.iwinvConfig.onIpRestrictionAlert === 'function') {
      try {
        await Promise.resolve(this.iwinvConfig.onIpRestrictionAlert(alertPayload));
      } catch (error) {
        console.warn('[iwinv] onIpRestrictionAlert callback failed:', error);
      }
    }

    if (typeof this.iwinvConfig.ipAlertWebhookUrl === 'string' && this.iwinvConfig.ipAlertWebhookUrl.length > 0) {
      try {
  ...
  private addIpRetryMetadata(result: StandardResult, maxAttempts: number): StandardResult {
    if (!result.error) {
      return result;
    }

    const currentMessage = result.error.message || '등록하지 않은 IP에서는 발송되지 않습니다.';
  ...
  private async sendSmsRequestOnce(
    ...
    const normalizedSender = senderNumber ? this.normalizePhoneNumber(senderNumber) : '';
    if (!normalizedSender) {
      return {
        messageId: `sms_error_${Date.now()}`,
        status: StandardStatus.FAILED,
    ...
    if (request.options?.scheduledAt instanceof Date && !Number.isNaN(request.options.scheduledAt.getTime())) {
    ...
    if (!secretHeader) {
      return {
        ...
        error: {
          ...
          message: 'SMS 인증키가 없습니다. IWINV_SMS_AUTH_KEY 또는 IWINV_SMS_API_KEY를 설정하세요.',
          retryable: false,
        },
      };
    }
    ...
    if (this.iwinvConfig.debug) {
      ...
      console.log(`[${this.id}] Request data:`, JSON.stringify(payload).substring(0, 200));
    }
    ...
    try {
      ...
      const message =
        typeof responseData.message === 'string' && responseData.message.length > 0
      ...
      return {
        messageId:
          (typeof responseData.requestNo === 'string' && responseData.requestNo.length > 0
            ? responseData.requestNo
            : typeof responseData.msgid === 'string' && responseData.msgid.length > 0
              ? responseData.msgid
...
export const createDefaultIWINVProvider = () => {
  const config: IWINVConfig = {
    ...
    senderNumber: process.env.IWINV_SENDER_NUMBER || process.env.IWINV_SMS_SENDER_NUMBER,
    sendEndpoint: process.env.IWINV_SEND_ENDPOINT || '/api/v2/send/',
    ipRetryCount: process.env.IWINV_IP_RETRY_COUNT ? Number(process.env.IWINV_IP_RETRY_COUNT) : undefined,
    ipRetryDelayMs: process.env.IWINV_IP_RETRY_DELAY_MS ? Number(process.env.IWINV_IP_RETRY_DELAY_MS) : undefined,
