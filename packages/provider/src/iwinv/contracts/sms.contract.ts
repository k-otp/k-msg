...
export class IWINVSMSContract implements SMSContract {
  ...
  async send(request: SMSSendRequest): Promise<SMSResult> {
    const messageType = this.determineMessageType(request.text, request.messageType);
    ...
    const payload = {
      ...
      ...(messageType === 'LMS' && request.subject && { subject: request.subject }),
      // 예약 발송시 YYYYMMDDHHMISS 형식
      reqdate: request.options?.scheduledAt ? this.formatDate(request.options.scheduledAt) : undefined
    };
    ...
    try {
      const response = await this.makeRequest<IWINVSMSResponse>('POST', endpoint, payload);
    ...
    for (const request of requests) {
      try {
        ...
        results.push({
          ...
          timestamp: new Date(),
          phoneNumber: request.phoneNumber,
          messageType: this.determineMessageType(request.text, request.messageType),
          error: {
            code: 'SEND_FAILED',
  ...
  async schedule(request: SMSSendRequest, scheduledAt: Date): Promise<ScheduleResult> {
  ...
  async cancel(messageId: string): Promise<void> {
    try {
      ...
      throw new Error(`Cancel failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  async getStatus(messageId: string): Promise<SMSStatus> {
    try {
      ...
      const response = await this.makeRequest<IWINVHistoryResponse>('POST', '/history/', {
        start_date: this.formatDateForHistory(new Date(Date.now() - 24 * 60 * 60 * 1000)), // 24시간 전
  ...
  // Helper methods
  private determineMessageType(text: string, explicitType?: 'SMS' | 'LMS'): 'SMS' | 'LMS' {
  ...
  private mapIWINVStatusToDeliveryStatus(code: string) {
    switch (code) {
      case '0': return { status: 'sent' as const, timestamp: new Date() };
      default: return { status: 'failed' as const, timestamp: new Date() };
    }
  }

  private mapIWINVStatusToSMSStatus(status: string): SMSStatus {
    switch (status) {
      case 'COMPLETE': return SMSStatus.DELIVERED;
      case 'SENDING': return SMSStatus.SENDING;
      case 'WAIT': return SMSStatus.QUEUED;
      case 'FAIL': return SMSStatus.FAILED;
  ...
  private async makeRequest<T = any>(method: string, endpoint: string, data?: Record<string, unknown>): Promise<T> {
    // IWINV SMS API 기본 URL: https://sms.bizservice.iwinv.kr/api/
    const baseUrl = this.config.baseUrl?.replace('/api/', '').replace('alimtalk.bizservice', 'sms.bizservice') || 'https://sms.bizservice.iwinv.kr';
    ...
    if (this.config.debug) {
      console.log(`IWINV SMS API ${method} ${url}:`, data ? JSON.stringify(data).substring(0, 200) : 'no body');
    }
...
export class IWINVSMSAccountContract implements SMSAccountContract {
  ...
  async getBalance(): Promise<SMSBalance> {
    try {
      // IWINV 실제 잔액 조회 API
      const response = await this.makeRequest<IWINVBalanceResponse>('POST', '/balance/', {});
  ...
  async getProfile(): Promise<SMSAccountProfile> {
    try {
      ...
      throw new Error(`Profile check failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  async getSenderNumbers(): Promise<SMSSenderNumber[]> {
    try {
      ...
      const response = await this.makeRequest<IWINVSenderResponse>('POST', '/sender/', {});
      ...
      return (response.list || []).map((sender) => ({
        ...
        verifiedAt: sender.approve_date ? new Date(sender.approve_date) : undefined,
        status: this.mapSenderStatus(sender.status),
        createdAt: new Date(sender.reg_date)
      }));
    } catch (error) {
      throw new Error(`Sender numbers check failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }

  async addSenderNumber(phoneNumber: string): Promise<SMSSenderNumber> {
    // IWINV는 발신번호 등록이 웹 콘솔에서만 가능
    throw new Error(`IWINV 발신번호 등록은 웹 콘솔에서만 가능합니다. https://sms.bizservice.iwinv.kr 에서 등록하세요.`);
  }

  async verifySenderNumber(phoneNumber: string, verificationCode: string): Promise<boolean> {
    ...
    try {
      ...
      throw new Error(`Verify sender number failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  private async makeRequest<T = any>(method: string, endpoint: string, data?: Record<string, unknown>): Promise<T> {
    ...
    if (this.config.debug) {
      console.log(`IWINV SMS Account API ${method} ${url}:`, data ? JSON.stringify(data).substring(0, 200) : 'no body');
    }
    ...
    if (this.config.debug) {
      console.log(`IWINV SMS Account API response:`, responseText.substring(0, 500));
    }
