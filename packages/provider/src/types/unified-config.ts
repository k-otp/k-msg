...
export function isValidBaseConfig(config: unknown): config is BaseConfig {
  return (
    ...
    ["development", "staging", "production"].includes(
      (config as BaseConfig).environment,
    )
  );
}
 ...
 * IWINV 기본 설정 검증
 */
export function isValidIWINVBaseConfig(
  config: unknown,
): config is IWINVBaseConfig {
  return (
    isValidBaseConfig(config) &&
    "apiKey" in config &&
    typeof (config as IWINVBaseConfig).apiKey === "string" &&
    "baseUrl" in config &&
    typeof (config as IWINVBaseConfig).baseUrl === "string"
  );
}
 ...
 * AlimTalk 설정 검증
 */
export function isValidAlimTalkConfig(
  config: unknown,
): config is AlimTalkConfig {
  return (
    isValidIWINVBaseConfig(config) &&
    "type" in config &&
    (config as AlimTalkConfig).type === "alimtalk"
  );
}
 ...
 * 통합 설정 검증
 */
export function isValidUnifiedConfig(
  config: unknown,
): config is UnifiedProviderConfig {
  return (
    isValidBaseConfig(config) &&
    typeof config === "object" &&
    config !== null &&
 ...
 */
export function toLegacyIWINVConfig(
  ...
  return {
    ...
    userId:
      (config.channels?.alimtalk?.userId as string | undefined) ||
      (config.channels?.sms?.userId as string | undefined),
    senderNumber:
      (config.channels?.alimtalk?.senderNumber as string | undefined) ||
      (config.channels?.sms?.senderNumber as string | undefined),
  };
}
 ...
 */
export function fromLegacyIWINVConfig(
  legacyConfig: IWINVBaseConfig,
