...
export const createTestRequests = () => ({
  ...
  sms: {
    short: {
      ...
      variables: {
        message: '[테스트] 인증번호는 ' + Math.floor(100000 + Math.random() * 900000) + '입니다.'
      }
    } as StandardRequest,
    ...
    long: {
      ...
      variables: {
        subject: '중요 공지사항',
        message: '안녕하세요. 이것은 긴 메시지 테스트입니다. '.repeat(10) + '감사합니다.'
      }
    } as StandardRequest
  },
...
// HTTP 응답 모킹 헬퍼
export const createMockHttpResponse = (options: {
  success?: boolean;
  delay?: number;
  statusCode?: number;
  data?: any;
...
export const createConcurrencyTester = () => {
  return {
    ...
    ): Promise<Array<{ result: T | null; error: Error | null; duration: number }>> => {
      const results: Array<{ result: T | null; error: Error | null; duration: number }> = [];
    ...
    testRateLimit: async <T>(
      ...
      duration: number
    ): Promise<{ attempted: number; successful: number; failed: number; actualRate: number }> => {
...
export const createTestStateTracker = () => {
  ...
  return {
    recordTest: (name: string, passed: boolean, duration: number, error?: string) => {
      state.totalTests++;
    ...
    getSummary: () => ({
      ...state,
      successRate: state.totalTests > 0 ? (state.passedTests / state.totalTests) * 100 : 0,
      averageDuration: state.totalTests > 0 ? state.duration / state.totalTests : 0
    }),
